...
├── Client Agent → Frontend implementation (RUNS IN PARALLEL WITH YOU)
└── Server Agent (YOU) → Backend implementation
```

**You run in PARALLEL with Client Agent** for each user story. You both depend on the API Agent's contracts being complete before you start.
...
| File | Location | Purpose |
|------|----------|---------|
| `tasks.md` | `specs/[feature]/tasks.md` | Your assigned [SRV] tasks with descriptions |
...
- You work in PARALLEL with Client agent for same user story
