...
/speckit.implement server  → Server Agent - Backend implementation (CAN RUN IN PARALLEL)
/speckit.implement client  → Client Agent (YOU) - Frontend implementation
/speckit.validate client   → Environment Validation Agent - Validates your work
```

**Separate Sessions**: You can run in a SEPARATE terminal/session from the Server Agent. Each agent works on its own task file (`tasks-client.md` vs `tasks-server.md`), avoiding conflicts.
...
| File | Location | Purpose |
|------|----------|---------|
| `tasks-client.md` | `specs/[feature]/tasks-client.md` | Your assigned tasks (Integration tests first) |
...
**IMPORTANT**: Do NOT edit tasks-client.md directly. The implement command handles task tracking.
...
**CRITICAL**: The implement command parses this JSON to update tasks-client.md. Always include it at the end.
...
- **Never edit tasks-client.md** - output JSON completion report instead
- You can run in a SEPARATE session from Server agent (different terminals)
